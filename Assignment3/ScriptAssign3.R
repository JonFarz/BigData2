...
##data <- read.dcf("C:\Python27\agaricus-lepiota.data")
...
model <- svm(EatIt ~ odor + habitat + population + bruises + CapShape + `spore-print-color`, data = trainData)
summary(model)

pred <- predict(model,testData[,-1])
tab <- table(pred = pred, true = testData[,1])

tab

#classAgreement(tab)

data_rpart <- rpart(EatIt ~. , data = trainData)

printcp(data_rpart)

data_pred <- predict(data_rpart, newdata = testData)
#table(data_pred, testData$EatIt)
...
#odor + habitat + population + bruises + CapShape + `spore-print-color` + `ring-type` + `ring-number` + `veil-color`+ `veil-type` + `stalk-color-below-ring` + `stalk-color-above-ring`
data_ctree <- ctree(EatIt ~ odor + habitat + population + bruises + CapShape + `spore-print-color` , data = trainData)

table(predict(data_ctree),trainData$EatIt)

plot(data_ctree)

testPred <- predict(data_ctree,newdata=testData)
table(testPred, testData$EatIt)
...
classifier <- naiveBayes(EatIt ~ odor + habitat + population + bruises + CapShape + `spore-print-color`, data = trainData)
pred <- predict(classifier,testData[,-1])
table(pred,testData$EatIt)
